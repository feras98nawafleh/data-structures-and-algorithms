...
const raisedToTheThird = (arr) => {
  let raisedToTheThirdArray = [];
  arr.forEach(number => {
    raisedToTheThirdArray.push(Math.pow(number, 3));
  });
...
const addOne = (arr) => {
  const addOneArray = arr.map(number => number + 1);
  return addOneArray;
};
...
const addQuestion = (arr) => {
  const addQuestionArray = arr.map(number => number + '?');
